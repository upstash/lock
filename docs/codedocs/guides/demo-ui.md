---
title: "Demo UI"
description: "Build a client UI that acquires a lock and visualizes the lease window."
---

This guide mirrors the demo app's client page. The UI calls the route handler from [Next.js Route Handler](./nextjs-route), displays whether the lock is acquired, and shows a countdown for the remaining lease time.
		...
		<main className="p-4 bg-white w-full h-full">
			<div className="w-full h-full">
				...
				<div className="text-center">
					...
					|
					<a href="https://github.com/upstash/lock/tree/main/examples/upstash-lock-demo" target="_blank" rel="noopener noreferrer" className="inline-flex items-center mx-2">
						<img src="github-mark.png" alt="GitHub" className="mr-1 h-4 w-4" />
						Demo
					</a>
				</div>
...
<Step>
### Connect it to the lock route
This page expects `/api/acquire-lock` to return `lockAcquired`, `leaseTime`, and `lockKey`. The route implementation is documented in [Next.js Route Handler](./nextjs-route).
</Step>
</Steps>

**Why this matters**
- The UI makes lock contention visible to users by showing per-request results.
- A live countdown illustrates how the lease time controls lock availability.
- Using a custom ID lets you test cross-device contention easily.

<Callout type="warn">The example embeds outbound links in the UI. If you remove them, ensure your UI still communicates where the lock behavior originates from.</Callout>
